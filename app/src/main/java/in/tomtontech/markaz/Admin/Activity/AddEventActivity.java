...
import static in.tomtontech.markaz.Admin.Adapter.CustomViewEventList.*;
public class AddEventActivity extends AppCompatActivity {
  private static final String LOG_TAG = "AddEvent";
  private EditText etName, etPlace, etTime, etDate, etDesc, etNumber;
  ...
  private void declareXml() {
    etDate = (EditText) findViewById(R.id.addEvent_eventDate);
    etPlace = (EditText) findViewById(R.id.addEvent_eventPlace);
    etTime = (EditText) findViewById(R.id.addEvent_time);
    etName = (EditText) findViewById(R.id.addEvent_eventName);
    etDesc = (EditText) findViewById(R.id.addEvent_eventDesc);
    etNumber = (EditText) findViewById(R.id.addEvent_eventContact);
    imPhoto=(ImageView)findViewById(R.id.addEvent_imageView);
  }

  public void onSubmitClicked() {
    String strName, strPlace, strTime, strDate, strNumber, strDesc;
    ...
    } else if (!validField(strNumber, Constant.TYPE_PHONE)) {

    } else if (!validField(strDate, Constant.TYPE_DATE)) {
      Toast.makeText(ctx, "Not In A Valid Format", Toast.LENGTH_SHORT).show();
    } else if (!validField(strDesc, Constant.TYPE_TEXT)) {

    } else if (!validField(strPlace, Constant.TYPE_TEXT)) {

    } else if (!validField(strTime, Constant.TYPE_TEXT)) {

    } else {
      AddEventAsync aea = new AddEventAsync(ctx);
      aea.execute(strName, strPlace, strDate, strTime, strNumber, strDesc,String.valueOf(id),base64String);
    }
  }
  ...
  private class getEventAsync extends AsyncTask<Integer,Void,String>
  {
    ...
    protected String doInBackground(Integer... integers) {
      ...
      try {
        String data =URLEncoder.encode("itemId", "UTF-8") + "=" + URLEncoder.encode(String.valueOf(itemId), "UTF-8");
        String fileName = "php_getEventDetail.php";
    ...
    protected void onPostExecute(String s) {
      ...
      else
      {
        try {
          ...
          {
            String strName=jo.getString("EventName");
            String strPlace=jo.getString("EventPlace");
            String strDesc=jo.getString("EventDesc");
            String strDate=jo.getString("EventDate");
            String strTime=jo.getString("EventTime");
            String strContact=jo.getString("EventContact");
            etDate.setText(strDate);
            etPlace.setText(strPlace);
            etName.setText(strName);
            etTime.setText(strTime);
            etDesc.setText(strDesc);
            etNumber.setText(strContact);
          }else
          {
