...
public class StaffLoginActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (!sp.getString(SP_USER, "").isEmpty()&&sp.getInt(SP_PRIVILAGE,0)!=5) {
      Intent intent = new Intent(ctx, ChatRoom.class);
      startActivity(intent);
      finish();
    } else if (sp.getInt(SP_PRIVILAGE,0)==5) {
  ...
  private class LoginAsync extends AsyncTask<String, Void, String> {
    ...
    protected void onPostExecute(String result) {
      ...
      if (result.equalsIgnoreCase("unsuccess")) {
        ...
        Toast.makeText(ctx, "OOPs! Something went wrong. Connection Problem.", Toast.LENGTH_LONG)
            .show();
      } else {
        try {
