...
public class RouteMapActivity extends FragmentActivity implements OnMapReadyCallback {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    int instId=getIntent().getIntExtra("inst_id",0);
    if(instId>0)
    {
  ...
  private class AsyncLocation extends AsyncTask<Integer,Void,String>
  {
    @Override
    protected void onPostExecute(String s) {
      ...
      {
        Toast.makeText(ctx, "Network Error Try Again Later.", Toast.LENGTH_SHORT).show();
      }else
      {
        try {
    ...
    protected String doInBackground(Integer... integers) {
      int inst_id=integers[0];
      try {
        String fileName = "php_getLocation.php";
        URL path = new URL(SERVER_ADDR.concat(fileName));
        String data= URLEncoder.encode("inst_id","UTF-8")+"="+URLEncoder.encode(String.valueOf(inst_id),"UTF-8");
